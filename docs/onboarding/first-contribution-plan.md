...
## Author
- Name / GitHub handle: Isaac Bown / @info3330ib
...
- From `docs/onboarding/repo-map.md`: Safe first-touch list (section 6) names `docs/`, `README.md`, and small scoped content changes as low risk. A new contributors file fits this category, it is docs-only and does not touch app code or config.
- From `docs/onboarding/team-orientation-notes.md`: Section 4 says a first PR is done when the change is scoped, isolated on a branch, described, reviewable, verified, and aligned with team conventions. It even names "a contributors list" as the kind of example that counts as scoped.
- From `docs/onboarding/ai-tooling-verification.md`: The GO decision recorded on 2026-09-09 confirms agent rules and context (repo-map, AGENTS.md, cursor rules) are verified enough to trust for drafting this plan and the implementation step after it.
...
2. Optional second touch (pick at most one, or none):
   - None chosen. This plan is constrained to one beginner-safe contribution, the `CONTRIBUTORS.md` change, so the first review stays small.
...
| File | Action | Why |
|------|--------|-----|
| CONTRIBUTORS.md | create | Add my contributor entry (no such file exists yet, per repo-map inventory) |
| n/a | n/a | No second touch chosen, single file change only |
